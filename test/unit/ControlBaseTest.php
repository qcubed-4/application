...
class ControlBaseTests extends \QCubed\Test\UnitTestCaseBase {
	...
	public function testAjaxChangeFormState() {
		if (!\QCubed\Project\Application::instance()->context()->cliMode()) {
			$this->assertEquals(2, self::$ctlTest->savedValue1, "Actions can change state for later queued actions.");
			$this->assertEquals(2, self::$ctlTest->savedValue2, "Actions can change state for later queued actions.");
			$this->assertNotEquals(1, self::$ctlTest->savedValue3, "Events can block later events until a response is received.");
		}
	}
