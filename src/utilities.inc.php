<?php
    ...
    function _p(string $strString, bool $blnHtmlEntities = true): void
    {
        // Standard Print
     ...
     *
     * The difference between _b() and _p() is that _b() will convert any linebreaks to <br/> tags.
     * This allows _b() to print any "block" of text that will have linebreaks in standard HTML.
     *
     * @param string $strString
     * @param boolean $blnHtmlEntities
     */
    function _b(string $strString, bool $blnHtmlEntities = true): void
    {
        // Text Block Print
        if ($blnHtmlEntities && (gettype($strString) != 'object'))
     ...
     */
    function _t(string $strString): void
    {
     ...
     * @return string
     */
    function _tr(string $strString): string
    {
        return t($strString);
    }
     ...
     */
    function _tp(string $strString, bool $blnHtmlEntities = true): void
    {
     ...
     * @return null|string
     */
    function _nl(?string $strText = null): ?string
    {
        if (Application::instance()->minimize()) {
            ...
            if (str_ends_with($strText, "\n")) {
                return $strText; // the text already ends with a newline
            } else {
                return $strText . "\n";
            }
        }
     ...
     * the purpose of indenting the text once to the right, so it is easier to read when viewing a source in a browser and
     ...
     * @return string
     */
    function _indent(string $strText, int $intCount = 1): string
    {
