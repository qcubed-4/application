<?php
     ...
     *  an event.target - the HTML object clicked in. If your table cell had other objects in it, this will return the
    ...
    class CellClick extends Click
    {
        ...
        const string ROW_INDEX = '$j(this).parent()[0].rowIndex';
        const string COLUMN_INDEX = 'this.cellIndex';
        const string CELL_ID = 'this.id';
        const string ROW_ID = '$j(this).parent().attr("id")';
        const string ROW_VALUE = '$j(this).parent().data("value")';
        const string COL_ID = '$j(this).parent().closest("table").find("thead").find("th")[this.cellIndex].id';
        ...
        public function __construct(?int $intDelay = 0, ?string $strCondition = null, mixed $mixReturnParams = null, ?bool $blnBlockOtherEvents = false)
        {
            ...
            if (!$mixReturnParams) {
                $this->strReturnParam = '{"row": $j(this).parent()[0].rowIndex, "col": this.cellIndex}'; // by default, returns the row and colum indexes of the cell clicked
