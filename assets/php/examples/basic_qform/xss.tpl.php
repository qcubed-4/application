...
<div id="instructions">
	...
	<p>
		...
		it <a href="http://en.wikipedia.org/wiki/Cross-site_scripting">on Wikipedia</a>.
    </p>

	<p>
        QCubed comes with two layers of protection against XSS. Both of these are enabled by default, and you don't
		need to do anything to make use of them.
    </p>

	<p>
        The first layer is around filtering input—particularly in <strong>TextBox</strong> controls. This is about filtering
	...
	<p>
        The default value used for creating TextBox instances can be altered by setting
		<strong>$strCrossScripting</strong> in /project/qcubed/Control/TextBox.php to one of the values above.
    </p>
	...
	<p>
		...
		an HTML tag, for example, &lt;script&gt; tag).
    </p>
	...
	<p>
		...
		the second protection (output filtering) and also list the tags that you want to allow by specifying:
    </p>
...
<div id="demoZone">
	...
	<p>
        <strong>Textbox protected with <code>TextBox::XSS_HTML_ENTITIES</code></strong>
    </p>
	...
	<p>
        <strong>Textbox protected with <code>TextBox::XSS_HTML_PURIFIER</code> with default settings.</strong>
    </p>
	<p><?php $this->txtTextbox3->render() ?></p>
	<p><?php $this->btnButton3->render(); ?></p>
	<p>&nbsp<?php $this->lblLabel3->render() ?></p>

	<p>
        <strong>Textbox protected with
            <code>TextBox::XSS_HTML_PURIFIER</code>
            with a set of tags that's allowed (ex.&lt;b&gt;, &lt;i&gt;). Note that you should make any change to the text
            in this input in order for it to be correctly processed. This is because of optimization made in the qcubed 3.0 version:
            HTML Purifier is designed to filter text coming from the browser, not from the PHP side.</strong>
    </p>
	...
	<p>
        <strong>Unprotected textbox (uses <code>TextBox::XSS_ALLOWED</code>). Not recommended—don't do this unless you have a good reason!</strong>
    </p>
