...
<div id="instructions">
	...
	<p>By including a self-updating TIMESTAMP column in your table, QCubed-4 can automatically
	...
	"midair collision.") Objects might go stale if two browsers are editing the same object in the database,
	...
	If they do not match, then it is safe to say that the data in the object is now stale, and QCubed-4
	...
	default will have the current time inserted into them and be automatically updated.
        ...
        QCubed-4 will automatically detect a column set up this way.</p>
	...
	<strong>Optimistic Locking</strong>. You can also tell QCubed-4 to automatically update such a column with
	the current timestamp. To do this, you enter a JSON expression into the comment field that sets the Timestamp and
	the AutoUpdate values to 1. You can find an example of how to do this in the <strong>pgsql.sql</strong> file for the
	example database. Essentially, you want your comment to contain this:
	<code>
    ...
    and templates that come with QCubed-4 handle it by informing the user that another user has simultaneously updated
    a record and asking if the user would like to overwrite those changes or reload the form to get the changes.</p>
...
<div id="demoZone">
	...
	<form method="post" action="<?php _p(Application::instance()->context()->scriptName()); ?>">
		...
		<p>Attempting to save Two Instances of the Same Object will throw an <strong>Optimistic Locking</strong> exception<br />
...
<?php
    ...
    // Note: on most web applications, because Optimistic Locking is more application
    // user-level constraints instead of systematic ones, this delay is inherited with the web
