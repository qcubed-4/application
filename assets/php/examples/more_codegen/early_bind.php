...
<div id="instructions">
    ...
    <p>When you need to perform <strong>loadAll()</strong> or <strong>loadArrayBy...()</strong> calls and would like to include related objects
        in order to reduce the number of database calls in an extended query, you can use QCubed's <strong>Object Expansion</strong> functionality to
    ...
    <p>Since the way we access the objects is the exact same, this
        ...
        the software to do or testing it with real people, and in doing so they can waste time overengineer some pieces of functionality.
        If the focus is on achieving application functionality first, then once the application is usable, you can profile the functionality
        that tends to be used more frequently and simply focus on optimizing that smaller, more a frequently used subset of functionality.</p>

    <p>You can expand from one table to another through foreign keys that reference a field in another table,
        through reverse references that have foreign keys that refer from the other table back to the table you
        are working with, or through association tables. Some of the other examples in this tutorial will teach
        you more about using each of these kinds of references.</p>
</div>

<div id="demoZone">
	<h2>List All Projects and its Manager</h2>
<?php
	// Enable Profiling (we're assuming the Example Site Database is at index 1)
	...
	foreach ($objProjectArray as $objProject) {
		_p(QString::htmlEntities($objProject->Name) . ' it is managed by ' . $objProject->ManagerPerson->FirstName . ' ' .
