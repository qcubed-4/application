...
<div id="instructions">
    ...
    <p>The QCubed-4 ORM (object-relation model) is the system that populates PHP objects with information from the
        ...
        a typed PHP object populated with the information from the query.</p>

    <p>By default, any object with related objects will perform-4
        "late binding" on that related object. This means that when you ask for a related object from a  ORM object,
         a database call is made at that moment and the object is created from the information returned by the database.<p>

    <p>In our <strong>Example Site Database</strong>, an example
    ...
    <p>In some cases, late binding can help limit traffic between the application and the database, but in others,
        ...
        and nothing else.  And fortunately, because the QCubed-4 generated code does the binding for you
    ...
    <p>In the example on this page, we call <strong>loadAll()</strong> to get all the <strong>Project</strong> objects
        and view each object's<strong>ManagerPerson</strong>. Using the built-in QCubed-4 Database Profiler, you can see that
...
<?php
	...
	foreach ($objProjectArray as $objProject) {
		_p('<li>'.$objProject->Name . ' it is managed by ' . $objProject->ManagerPerson->FirstName . ' ' .
