...
<div id="instructions">
    ...
    <p>The state data of the control is saved in the session by default in a variable named the configuration constant
        <b>QCUBED_SESSION_SAVED_STATE</b> (the default value is 'QSavedState'). If your application has authenticated
        users who can see private data, you can serialize the contents of this session variable and store it in your
        database with the user's credentials, and then restore it on the next login. The user will then see your application
        in the same state as before. The data stored in the session variable is only necessary to restore the visual state of
        the control and takes up very little space.</p>
    ...
    <h2>Note on Usage</h2>
    <p>The moment that you set <b>SaveState</b> to <b>true</b>, QCubed-4 will look for a previously saved
...
<div id="demoZone">
    ...
    <fieldset>
        ...
        <p><?php $this->ddnProjectPicker1->render(); ?></p>
        <p><?php $this->fld1->render(); ?></p>
    </fieldset>
    <fieldset>
        <legend>SavedState = true</legend>
        <p><?php $this->ddnProjectPicker2->render(); ?></p>
        <p><?php $this->fld2->render(); ?></p>
    </fieldset>
    <p><?php $this->btnReload->render(); ?></p>
