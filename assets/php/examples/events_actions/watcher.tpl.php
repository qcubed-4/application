...
<?php $this->renderBegin(); ?>
    ...
    <div id="instructions">
        ...
        <p>The <strong>Watcher</strong> class is used to connect a control to a database table or tables so that
            ...
            set up callbacks between edit forms and dialogs in order to refresh a control that is viewing data. In
            addition,
            in a multi-user environment, when one user changes the data, the other user will automatically see the
            change. This is similar to what you might see in a system like Node.js, with some caveats</p>

        <p>The current implementation requires the browser to generate either an Ajax event or Server event in order to
            detect the change. In a multi-user environment, if the user is actively using your application, this should
            happen pretty often. However, if your application is such that the user might have long periods of
             inactivity but still should see the results of activity from other users, you can do a couple of things: </p>
        <ul>
            <li>
                Set up a <strong>JsTimer</strong> to generate periodic events. See the <a href="../other_controls/timer_js.php">JsTimer
                    example page</a> for help. On that example page, it discusses adding actions to the timer. For purposes
                of generating opportunities for the Watcher to look at the database, you will add a null ajax action to the timer.
            </li>
            <li>
                The other option, which is currently not implemented in QCubed-4, is to create a direct connection between
                the server and the user's browser that will trigger these events. There are a few different technologies to
                do this, and many require a customized HTML server. Apache will not do this out of the box. A messaging server
    ...
    <div id="demoZone">
        ...
        <p>First:<?php $this->txtFirstName->renderWithError(); ?></p>
        <p>Last:<?php $this->txtLastName->renderWithError(); ?></p>
