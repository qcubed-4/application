...
<div id="instructions">
    <h1>Using the DataGridCheckboxColumn</h1>

    <p>In this example we will take our Paginated <strong>DataGrid</strong>, and add a column which has a
    ...
    unknowingly replace values recently changed by another user. It's better to have real-time awareness of the values when two
    people are editing the same data.</p>

    <p>The <strong>DataGridCheckboxColumn</strong> is designed to help manage a column of checkboxes.
    ...
    <h2>Check All</h2>
    <p>
        The <strong>DataGridCheckboxColumn</strong> can display a checkbox in the header that will check all, or check none,
        by setting the <strong>ShowCheckAll</strong> property to true.
        In order to implement this, <strong>DataGridCheckboxColumn</strong> must be able to get an id
        for each possible row shown, so that when the user pages through data, they can see what is checked. In order to do this,
        you must subclass <strong>DataGridCheckboxColumn</strong> and implement the <strong>getAllIds()</strong> method.
    </p>

    <hr>

    <p>
        In the first example, we are using the default functionality of the <strong>DataGridCheckboxColumn</strong> class
        to create a selection list that will be acted on later. The <strong>DataGridCheckboxColumn</strong> keeps a record
        of what is checked in the _SESSION variable. It uses a subclass of <strong>DataGridCheckboxColumn</strong> to get
    ...
    with the ACME project. To support this type of interaction, we subclass <strong>DataGridCheckboxColumn</strong> and
