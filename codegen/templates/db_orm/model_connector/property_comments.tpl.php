...
foreach ($objTable->ColumnArray as $objColumn) {
    ...
    if (!$blnIsLabel && (!isset($objColumn->Options['FormGen']) || $objColumn->Options['FormGen'] != Options::FORMGEN_LABEL_ONLY)) { ?>
 * @property <?= $strClassName; ?> $<?= $objColumn->PropertyName ?>Control
<?php }
    if ($blnIsLabel || !isset($objColumn->Options['FormGen']) || $objColumn->Options['FormGen'] != Options::FORMGEN_CONTROL_ONLY) { ?>
...
foreach ($objTable->ReverseReferenceArray as $objReverseReference) {
    if (!$objReverseReference->Unique || (isset($objReverseReference->Options['FormGen']) && $objReverseReference->Options['FormGen'] == Options::FORMGEN_NONE)) continue;
    $strClassName = $objCodeGen->GetControlCodeGenerator($objReverseReference)->GetControlClass();
    $blnIsLabel = ($strClassName == 'QCubed\\Control\\Label');

    if (!$blnIsLabel && (!isset($objReverseReference->Options['FormGen']) || $objReverseReference->Options['FormGen'] != Options::FORMGEN_LABEL_ONLY)) { ?>
 * @property <?= $strClassName; ?> $<?= $objReverseReference->ObjectDescription ?>Control
<?php }
    if ($blnIsLabel || !isset($objReverseReference->Options['FormGen']) || $objReverseReference->Options['FormGen'] != Options::FORMGEN_CONTROL_ONLY) { ?>
...
<?php foreach ($objTable->ManyToManyReferenceArray as $objManyToManyReference) {
    if (isset($objManyToManyReference->Options['FormGen']) && $objManyToManyReference->Options['FormGen'] == Options::FORMGEN_NONE) continue;
        ...
        if (!$blnIsLabel && (!isset($objManyToManyReference->Options['FormGen']) || $objManyToManyReference->Options['FormGen'] != Options::FORMGEN_LABEL_ONLY)) { ?>
 * @property <?= $strClassName; ?> $<?= $objManyToManyReference->ObjectDescription ?>Control
<?php }
        if ($blnIsLabel || !isset($objManyToManyReference->Options['FormGen']) || $objManyToManyReference->Options['FormGen'] != Options::FORMGEN_CONTROL_ONLY) { ?>
